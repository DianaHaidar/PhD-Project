...
source('iForest.update.FP.chunk.oversample.ver2.R')
...
foreach(ascore.thresh= ascore.thresh.vec, .combine=cbind) %:%
  foreach(FP.chunk.size= FP.chunk.size.vec, .combine='c') %dopar%   {
    ...
    print(i)
    print(paste( nt,phi.param,ascore.thresh,FP.chunk.size,perc.over,sep=','))
    iForest.update.FP.chunk.oversample(data, nt,phi.param,ascore.thresh,FP.chunk.size,perc.over) 
...
#       iForest.update.FP.chunk.oversample(data,kernel,FP.chunk.size,perc.over)
#     }
