ensemble.iForest.update.FP.chunk.oversample <- function(data,e.size,kmeans.method, nt,phi.param,ascore.thresh,FP.chunk.size,perc.over) {
  
  main.dir <- 'C:/Users/research/Documents/RStudio/workspace/oneclass.test'
  sub.dir <- paste(e.size,kmeans.method, nt,phi.param,ascore.thresh,FP.chunk.size,perc.over,sep=',')
...
for(f.test in folds.data)
{
  ...
  iForest.pred.bool <- array(FALSE,c(dimT,e.size)) 
  cluster <- rep(list(NULL),e.size)
  iForest.model <- rep(list(NULL),e.size)
  ...
  for(e in 1:e.size)
  {
    ...
    iForest.model[[e]] <- iForest(cluster[[e]][,1:16], nt=nt , phi=phi , seed=42)
    
  }
  ######################################################
  #step1: test instance after instance
  iForest.pred <- array('',c(dimT,1))
  ...
  for(inst in 1:dimT)
  {
    ...
    #calculate iForest.pred.bool for each test instance
    #save for vote
    library(caret)
    ...
    for(e in 1:e.size)
    {
      ...
      iForest.pred.ascore <- predict(iForest.model[[e]], test.instance[1,1:16])
      iForest.pred.bool[inst,e] <- iForest.pred.ascore > ascore.thresh
    }
    ...
    if(test.instance.label=='Anomalous' && iForest.pred[inst, ] =='Anomalous')
      TP <- TP + 1
    #no 'Normal' class label in ensemble
    if(test.instance.label!='Anomalous' && iForest.pred[inst, ] =='Anomalous')
      FP <- FP + 1
    if(test.instance.label=='Anomalous' && iForest.pred[inst, ] =='Normal')
      FN <- FN + 1
    if(test.instance.label!='Anomalous' && iForest.pred[inst, ] =='Normal')
      TN <- TN + 1
    ...
    if(test.instance.label!='Anomalous' && iForest.pred[inst, ]=='Anomalous')
    {
        ...
        if((FP!=0) && (FP%%FP.chunk.size ==0))
        {
          ...
          cluster.dim <- dim(cluster.data.append.syn.data[[e]])
          if(phi.param==0){
            phi <- cluster.dim[1]
          }else{
            phi <- phi.param
          ...
          set.seed(42)
          iForest.model[[e]] <- iForest(cluster.data.append.syn.data[[e]][,1:16], nt=nt , phi=phi , seed=42)
