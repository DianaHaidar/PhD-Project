ocsvm.update.FP.chunk.oversample <- function(data,kernel,FP.chunk.size,perc.over) {
  
  main.dir <- 'C:/Users/research/Documents/RStudio/workspace/oneclass.test'
  sub.dir <- paste(kernel,FP.chunk.size,perc.over,sep=',')
...
for(f.test in folds.data)
{
  ...
  for(inst in 1:dimT)
  {
    ...
    if((FP!=0) && (FP%%FP.chunk.size ==0))
    {
      ...
      set.seed(42)
      svm.model<-svm(train.data.append.syn.data[,1:16],y=NULL,
                     type='one-classification',
                     nu=0.5,
                     scale=TRUE,
                     kernel=kernel)
