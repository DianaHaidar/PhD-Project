...
commMatrixThreatLabel=cell(nbr_rows,1);
list = dir('r5.2-*-*.xlsx');
    ...
    for m=1:numel(malRecords)
        ...
        for s=2:nbr_rows
            ...
            if isempty(lTrue)==0
                ...
                name=list(f).name;
                sName = split(name,'-');
                insider=split(sName(3),'.');
                threatLabel=strcat('s',sName(2),'_',insider(1));
                commMatrixThreatLabel{s,1} = threatLabel;
                break;
            else
                fprintf(' ');
            end 
...
fprintf('\nWriting into file commMatrixThreatLabel.xlsx...');
xlswrite('commMatrixThreatLabel', commMatrixThreatLabel);
